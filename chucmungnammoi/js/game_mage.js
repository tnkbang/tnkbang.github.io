...
var init_close = function() {
    btn_close = document.createElement("close");
    btn_close.style.position = 'fixed';
    btn_close.style.top = '0';
    btn_close.innerHTML = '<button id="dong_game_mage" type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
    document.querySelector('.game_mage').appendChild(btn_close);
};

var update_fps = function() {
    fps.innerHTML = 'fps: ' + Math.round(1000/(Date.now()-now));
    now = Date.now();
};
...
var init_fps = function() {
    ...
    fps.style.top = '30px';
    document.querySelector('.game_mage').appendChild(fps);
    now = Date.now();
};

var init_canvas = function() {
    c   = document.getElementById("game_mage");
    ctx = c.getContext("2d");
...
$('#game').on('click',function(){
    if(!open){
        ...
        requestAnimationFrame(pull_loop);
        open = true;
        $('#dong_game_mage').on('mousemove',function(){
            toast.makeToast("Thoát game");
        })
        $('#dong_game_mage').on('click',function(){
            $('#main_web').show();
            document.querySelector('.game_mage').style.display = 'none';
        })
    }
    $('#main_web').hide();
    document.querySelector('.game_mage').style.display = 'block';
})
...
var main_loop, fps, btn_close, now, c, ctx, W, H;
...
questionmark.add_event_listener( "click" , function() {
    toast.makeToast("Đánh bại tất cả kẻ thù !<br>Tiêu tốn số lượng xu tương ứng để mua máu và năng lượng trong shop.");
});
...
function Minimap(map, colors) {
    ...
    mini.addEventListener("click", function(e) {
        if(canvas.parentNode != document.querySelector('.game_mage'))
            document.querySelector('.game_mage').appendChild(canvas);
        else document.querySelector('.game_mage').removeChild(canvas);
    ...
    */
    ...
    document.querySelector('.game_mage').appendChild(mini);
...
function Joystick(callback) {
    ...
    var setup = function() {
        ...
        for(var i in divs) {
            ...
            document.querySelector('.game_mage').appendChild(divs[i]);
        ...
        var change_func = function(e) {
            if(is_targeted(e.target, change_div)) {
                ...
                if(color === "rgb(0, 0, 0)") {
                    pause();
                    toast.makeToast("Kéo các nút điều khiển để thay đổi vị trí của chúng !");
        ...
        document.querySelector('.game_mage').appendChild(change_div);
...
function Element(design, text) {
    ...
    this.show = function() {
        if(!this.on_screen()) {
            document.querySelector('.game_mage').appendChild(this.div);
    ...
    this.on_screen = function(node) {
        node = node || this.div;
        if(node.parentNode) {
            var a = node.parentNode == document.querySelector('.game_mage');
    ...
    this.copy_text = function(text) {
        ...
        document.querySelector('.game_mage').appendChild(textArea);
        textArea.focus();
        textArea.select();
        document.execCommand('copy');
        document.querySelector('.game_mage').removeChild(textArea);
    };
...
var load_level = function(id) {
    ...
    if(lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
        ...
        toast.makeToast("Bạn cần "+lvl.cost
        +" xu để vào cấp độ "+id+"!");
    } else {
        toast.makeToast("Cấp độ "+id+" chưa tồn tại !");
    }
};
